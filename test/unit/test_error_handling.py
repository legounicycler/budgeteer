...
def test_bug_report_send_failure_malformed_textual_data(logged_in_user_client, mock_get_uuid_from_cookie_error_handling):
    ...
    assert response.status_code == 200
    assert b'{"error":{"bug_reporter_name":["This field is required."]},"success":false}' in response.data
    ...
    assert response.status_code == 200
    assert b'{"error":{"bug_reporter_email":["This field is required."]},"success":false}' in response.data
    ...
    assert response.status_code == 200
    assert b'{"error":{"bug_description":["This field is required."]},"success":false}' in response.data
    ...
    assert response.status_code == 200
    assert b'{"error":{"bug_reporter_name":["Field must be between 1 and 300 characters long."]},"success":false}' in response.data
    ...
    assert response.status_code == 200
    assert b'{"error":{"bug_reporter_email":["Invalid email address."]},"success":false}' in response.data
