...
def set_secure_cookie(response, key, value):
  """
  Encrypts the user's uuid and converts it to a secure cookie which it adds to the response
  The cookie is used for ensuring that users can only make changes to transactions in their own account.

  ARGS:
  response: Flask response object to which the cookie will be added
  key: string with name 'uuid'
  value: string with the user's uuid
...
def get_uuid_from_cookie():
  """
  Retrieves the user's uuid from the secure cookie set by set_secure_cookie()
  This cookie is used for ensuring that users can only make changes to transactions in their own account
  """
...
def generate_token(email):
  """
  Generates a secure token representing the given email address using the timed serializer.
  The token sent to the user in an email as part of a link for actions like confirming their email address or resetting their password.
  """
  return current_app.extensions['timed_serializer'].dumps(email, salt=current_app.config["SECURITY_PASSWORD_SALT"])

def confirm_token(token, expiration=3600):
  """
  Used for confirming a token representing an email generated by generate_token()
  If the token is successfully decrypted, it returns the email addrress used to retrieve the user object from the database
  and either reset their password or confirm their email address.
  """
  try:
...
def verify_recaptcha(recaptchaToken):
  """
  Verifies the reCAPTCHA token by sending a POST request to the reCAPTCHA verification URL.
  """
